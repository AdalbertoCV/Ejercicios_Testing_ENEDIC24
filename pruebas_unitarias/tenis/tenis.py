...
class tenis:
    ...
    def iniciar(self, p1, p2):
        # inicializamos los nombres de los juagadores
        self.players = [p1, p2]
    ...
    def points(self, player):
        ...
        # si tiene 40 puntos
            ...
            verificarEmpate_Ventaja(self)

# funcion para asignar a los jugadores en el turno actual


def asignarJugadores(self, player):
...
# funcion para verificar el estado del marcador,
# si es empate o ventaja para algun jugador


def verificarEmpate_Ventaja(self):
    ...
    if self.scores[self.rival_Actual] == 40:
        ...
        if self.ventaja == self.rival_Actual:
            ...
            if self.ventaja == self.player_Actual:
                ...
                self.resultado = self.players[self.player_Actual] + \
                    " Win"
            else:
                # si no tenemos la ventaja, se otorga la ventaja
                self.ventaja = self.player_Actual
                self.resultado = "Ventaja " + \
                    self.players[self.player_Actual]
    else:
