...
""" Ejercicio 29: Determinar la cantidad de dinero que recibirá
un trabajador de las horas extra, sabiendo que cuando las horas
trabajadas exceden las 40, el resto es horas extras y se pagan al
doble de una normal, y si exceden de 8 horas extra,
...
class horas_extra():
    def calcular(self, horas, costo):
        ...
        if (isinstance(horas, int) or
            isinstance(horas, float)) and (isinstance(costo, int) or
                                           isinstance(costo, float)):
            ...
            if not isinstance(horas, bool) and not isinstance(costo, bool):
                ...
                if isinstance(horas, int):
                    ...
                    # Si se exceden las 8 horas extra, entonces son triples
                        ...
                        # Asilamos las horas dobles
                        # de las triples y las normales
                        triples = horas-48
                        dobles = 8
                        normales = 40
                        return (normales * costo) + \
                            (dobles * (costo*2)) + (triples * (costo*3))
                else:
